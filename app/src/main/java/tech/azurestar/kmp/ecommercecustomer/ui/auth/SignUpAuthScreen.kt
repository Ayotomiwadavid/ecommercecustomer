...
@Composable
fun SignUpAuthScreen(navController: NavController) {
                ...
                AuthForm(state = state) {
                    ...
                    Column(
                        ...
                        Button(
                            ...
                            modifier = Modifier
                                .fillMaxWidth()
                                .padding(horizontal = 10.dp),//Login with email and password,
                            enabled = state.validForm,
                        ) {
                            Text(TextProvider.SIGN_UP.getText())
                        }
                        TextButton(onClick = {
                            navController.navigate(NavLocations.AUTH_LOGIN.name) {
                                popUpTo(
                                    0
                                )
                            }
                        }) {
                            Text(TextProvider.LOGIN_INSTEAD.getText())
                        }
                    }
                }
            }
